...
std::string get_error_description(int id) {
  switch(id){
...
device::types::MediaType::type to_media_type(int t) {
  switch (t) {
  case FRAMETYPE_H:   return device::types::MediaType::FileHeader;
  case FRAMETYPE_V:   return device::types::MediaType::VideoFrame;
  case FRAMETYPE_A:   return device::types::MediaType::AudioData;
  default:            return device::types::MediaType::MediaData;
  }
}
...
void Server::InitSDK(){
  ...
  auto disconnect_callback = [](gt_dev_handle_t login_handle) -> int {
    std::cout << "receive dvr " << login_handle << " disconnect notify" << std::endl;
  };
...
void Media::PlayBackByTime(){
  ...
  // auto handle = gt_require_pb_av_service((long)_login_id, channel_id, NSPEED, "0.0.0.0",&start, &end,data_callback, finish_callback);
  // if (handle < 0) {
  // }
}
